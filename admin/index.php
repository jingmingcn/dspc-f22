...
if ($use_auth) {
    if (isset($_SESSION[FM_SESSION_ID]['logged']) &&
     ( isset($auth_users[$_SESSION[FM_SESSION_ID]['logged']])  ||  null !== $userStore->findOneBy(['num','=',$_SESSION[FM_SESSION_ID]['logged']])
     ) ) {
        // Logged
        ...
        if(function_exists('password_verify')) {
            if (
            (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']])) 
            ||
            (password_verify($_POST['fm_pwd'], ($user = $userStore->findOneBy(['num','=',$_POST['fm_usr']]))?$user['password']:''))
            ) {
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    // Change Password
    if (isset($_POST['type']) && $_POST['type'] == "changepwd" && isset($_POST['newpwd'])) {

        $pwd = $_POST['newpwd'];
        $user = $userStore->findOneBy(['num','=',$_SESSION[FM_SESSION_ID]['logged']]);
...
if (isset($_GET['changepwd']) && !FM_READONLY) {
    ...
    <div class="col-md-8 offset-md-2 pt-3">
        <div class="card mb-2 <?php echo fm_get_theme(); ?>">
            <h6 class="card-header">
                <i class="fa fa-cog"></i>  <?php echo lng('Change Password') ?>
                <a href="?p=<?php echo FM_PATH ?>" class="float-right"><i class="fa fa-window-close"></i> <?php echo lng('Cancel')?></a>
            </h6>
            <div class="card-body">
                <form id="js-changepwd-form" action="" method="post" data-type="ajax" onsubmit="return change_password(this)">
                    <input type="hidden" name="type" value="changepwd" aria-label="hidden" aria-hidden="true">
                    <div class="form-group row">
                        <label for="newpwd" class="col-sm-3 col-form-label"><?php echo lng('Input New Password') ?></label>
                        <div class="col-sm-5">
                            <input type="password" class="form-control btn-sm" id="newpwd" name="newpwd" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-sm-10">
                ...
                <ul class="navbar-nav mr-auto float-right <?php echo fm_get_theme();  ?>">
                    ...
                    <li class="nav-item avatar dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            ...
                            <a title="<?php echo lng('Change Password') ?>" class="dropdown-item nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;changepwd=1"><i class="fa fa-lock" aria-hidden="true"></i> <?php echo lng('Change Password') ?></a>
                            <?php endif ?>
...
<head>
    ...
    <link rel="stylesheet" href="libs/bootstrap/4.5.3/css/bootstrap.min.css">
    <style>
...
<script src="libs/bootstrap/4.5.3/js/bootstrap.min.js"></script>
...
function lng($txt) {
    ...
    $tr['en']['Input New Password'] = 'Input New Password';
    $tr['en']['Input New Password Again'] = 'Input New Password Again';
